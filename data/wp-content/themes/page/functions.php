...
function autoback_my_mail($Mail_raw, $values, $Data)
{
  if ($Data->get('ご予約レストラン') == '洋食レストラン　デルソーレ') {
    ...
    $Mail_raw->cc = 'y-ide@h-kyocera.co.jp';
    ...
    $Mail_raw->to = 'd-yokoyama@h-kyocera.co.jp';
    $Mail_raw->cc = 't-fujita@h-kyocera.co.jp';
    $Mail_raw->bcc = 'okuda@arionet.jp';
...
// イベント予約フォーム 送信完了後に完了ページへ遷移
function my_mwform_after_send($Data)
{
  $form_key = $Data->get_form_key();
  if ($form_key === 'mw-wp-form-26848') {
    wp_redirect('/events-reservation-thanks/');
    exit;
  }
}
add_action('mwform_after_send_mw-wp-form-26848', 'my_mwform_after_send');
